...
namespace MVVM_Person.ViewModels
{
    public class PersonViewModel : ViewModelBase
    {
        ...
        public PersonViewModel()
        {
            if (File.Exists(path))
            {
                Lista_osob = new ObservableCollection<Person>(Serializacja_wczytaj_zapisz.Wczytaj(path));
            }
        }
        ...
        private void Dodaj()
        {
            var footballer = new Person(BoxName, BoxlastName, SliderWeight, SliderAge);
        ...
        private void Edytuj()
        {
            ...
            if (dialogResult == MessageBoxResult.Yes)
            {
                Lista_osob.Insert(Id, footballer);
                Id -= 1;
        ...
        public ICommand IC_wczytaj
        {
            get
            {
                if (wczytaj == null)
                {
                    ComandList comandList = new ComandList(arg => Lista_osob = new ObservableCollection<Person>(Serializacja_wczytaj_zapisz.Wczytaj(path)),arg => File.Exists(path));
                    wczytaj = comandList;
                }
                return wczytaj;
            }
        ...
        public ICommand IC_zapisz
        {
            get
            {
                if (zapisz == null)
                {
                    ComandList comandList = new ComandList(arg => Serializacja_wczytaj_zapisz.Zapisz(path, Lista_osob.ToList()),arg => true);
                    zapisz = comandList;
                }
                return zapisz;
            }
